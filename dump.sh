...
# Captures C++, Shaders, SQL schema, CMake, and the vcpkg manifest
find . -type f \( -name "*.cpp" -o -name "*.hpp" -o -name "*.h" -o -name "*.vert" -o -name "*.frag" -o -name "*.sql" -o -name "CMakeLists.txt" -o -name "vcpkg.json" \) \
-not -path '*/.*' -not -path './build*' -not -path './vcpkg*' -not -path './vcpkg_installed*' | while read -r file; do
    ...
    if [[ "$filename" == "CMakeLists.txt" ]]; then
        ...
        echo '```sql' >> project_context.md
    elif [[ "$filename" == "vcpkg.json" ]]; then
